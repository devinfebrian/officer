...
class Guardrail:
    ...
    def check(self, sender: str, recipient: str, message: str):
        for name in self.config.screens:
            detail = self._run_screen(name, recipient, message)
            if detail is not None:
                return False, WriteUp(screen=name, detail=detail, source_role=sender)
        return True, None

    def _run_screen(self, name: str, recipient: str, message: str):
        if name == "injection":
        ...
        if name == "role_policy":
            ...
            return screen_functions.role_policy_screen(message, allowed)
        return None
